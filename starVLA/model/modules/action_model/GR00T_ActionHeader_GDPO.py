...
class FlowmatchingActionHead(nn.Module):
    ...
    def predict_action(
        ...
        """
        Generate an action chunk by iterative denoising / flow integration.

        Args:
            vl_embs: [B, L, C] VLM embeddings
            state:  optional state tensor
            z:      optional initial noise actions, shape [B, H, A]
                    - if None: sample N(0, I) (stochastic)
                    - if given: deterministic given z
            num_steps: optional override of inference timesteps (speed control)
        Returns:
            actions: [B, H, A]
        """
        ...
        if z is None:
            ...
            assert z.shape == (batch_size, H, A), f"z must be {(batch_size, H, A)}, got {z.shape}"
            actions = z.to(device=device, dtype=vl_embs.dtype)
        ...
        if num_steps <= 0:
            return actions  # degenerate fallback
    ...
    def predict_mu(
        ...
        """
        Deterministic action chunk for GDPO (a 'mean-like' reference output).
        """
        B = vl_embs.shape[0]
        H = self.action_horizon
        A = self.action_dim
        ...
        if z_mode == "zeros":
            z = torch.zeros((B, H, A), device=device, dtype=dtype)
        elif z_mode == "fixed_rand":
            g = torch.Generator(device=device)
            g.manual_seed(int(fixed_seed))
            z = torch.randn((B, H, A), generator=g, device=device, dtype=dtype)
        else:
            raise ValueError(f"Unsupported z_mode={z_mode}")

        return self.predict_action(vl_embs=vl_embs, state=state, z=z, num_steps=num_steps)
    ...
    def sample_actions(
        ...
        """
        Sample a group of action chunks for GDPO/GRPO.
        ...
        # repeat conditioning
        vl_rep = vl_embs.repeat_interleave(num_samples, dim=0)  # [B*K, L, C]
        if state is not None:
            state_rep = state.repeat_interleave(num_samples, dim=0)
        else:
            state_rep = None

        # sample different z for each sample
        z = torch.randn((B * num_samples, H, A), device=device, dtype=dtype)

        actions = self.predict_action(vl_rep, state_rep, z=z, num_steps=num_steps)  # [B*K, H, A]
        actions = actions.view(B, num_samples, H, A)
        return actions
